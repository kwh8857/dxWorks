const prevBtn = document.querySelector(".id-interview-btn-prev");
const nextBtn = document.querySelector(".id-interview-btn-next");
const slideList = document.querySelector(".id-interview-list");
const pageItems = document.querySelectorAll(".id-interview-page-item");
const slides = document.querySelectorAll(".id-interview-item");
const slideContainer = document.querySelector(".id-interview-list-container");
let originalSlides = Array.from(slideList.children).filter((el) =>
  el.classList.contains("id-interview-item")
);
...
export const mbInterview = () => {
  ...
  // 초기화: 슬라이드 복제 제거
  const slides = slideList.querySelectorAll(".id-interview-item");
  if (slides.length > 3) {
    slideList.innerHTML = "";
